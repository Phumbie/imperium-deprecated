    ...
    <div v-if="!loading">
      <section class="top-section">
        ...
        <div class="products-container">
          ...
          <div class="product-item" v-for="invert in inverterFill"></div>
...
<script>
  ...
  export default {
    ...
    watch: {
      panels(val) {
        if (val.length < 2) {
          this.panelFill = 2 - val.length;
        }
      },
    },
    computed: {
      inverterFill() {
        if (this.inverters.length < 2) {
          return 2 - this.inverters.length;
        }
      },
      fillBatteries() {
        if (this.batteries.length < 2) {
          return 2 - this.batteries.length;
        }
      },
      fillAccessory() {
        if (this.accessories.length < 2) {
          return 2 - this.accessories.length;
        }
      },
    },
    methods: {
      ...
      fetchProducts() {
        ...
        api.getSimilarProducts("solar panel", 100000).then((response) => {
          if (response.data.data.result.length > 2) {
            this.panels = response.data.data.result.slice(-2);
            this.loading = false;
          } else {
            this.panels = response.data.data.result;
            this.loading = false;
          }
          // console.log(this.panels, this.panels.length);
          // this.panels = response.data.data.result;
          // if (response.data.data.result.length < 2) {
